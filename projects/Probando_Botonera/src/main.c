...
static Botonera statechart;

/*! This is a timed state machine that requires timer services */
#define NOF_TIMERS (sizeof(BotonTimeEvents)/sizeof(sc_boolean))+(sizeof(LedTimeEvents)/sizeof(sc_boolean))
 ...
 */
void led_opLED( Led* handle, const sc_integer LEDNumber, const sc_boolean State)
//void botoneraIface_opLED( Botonera* handle, sc_integer LEDNumber, sc_boolean State )
{
...
void boton_set_timer(Boton* handle, const sc_eventid evid, const sc_integer time_ms, const sc_boolean periodic)
{
	SetNewTimerTick(ticks, NOF_TIMERS, evid, time_ms, periodic);
}
void led_set_timer(Led* handle, const sc_eventid evid, const sc_integer time_ms, const sc_boolean periodic)
{
	SetNewTimerTick(ticks, NOF_TIMERS, evid, time_ms, periodic);
}
 ...
 *  \param evid An unique identifier of the event.
 */
void boton_unset_timer(Boton* handle, const sc_eventid evid)
{
	UnsetTimerTick( ticks, NOF_TIMERS, evid );
}
void led_unset_timer(Led* handle, const sc_eventid evid)
{
	UnsetTimerTick( ticks, NOF_TIMERS, evid );
}
...
//	uint8_t RET = Board_TEC_GetStatus(BOARD_TEC_1);
//	uint8_t ret = false;
//	uint32_t idx;
//
//	for (idx = 0; idx < 4; ++idx) {
...
//	}
//	return ret;
	return !Board_TEC_GetStatus(BOARD_TEC_4);
}
...
int main(void)
{
	...
	botonera_init( &statechart );
	botonera_enter( &statechart );
		...
		if (SysTick_Time_Flag == true) {
			...
			for (i = 0; i < NOF_TIMERS; i++) {
				...
				if (IsPendEvent( ticks, NOF_TIMERS, ticks[i].evid ) == true) {
					...
					boton_raise_time_event(statechart.internal.B1, ticks[i].evid );
					led_raise_time_event(statechart.internal.L1, ticks[i].evid );
			...
			if (BUTTON_Status != 0)	botoneraIface_raise_ev_button_PRESS(&statechart);
			else					botoneraIface_raise_ev_button_RELEASE(&statechart);
			/* Then Run an Cycle of Statechart */
			botonera_runCycle(&statechart);		// Run Cycle of Statechart
		}
	}
}
